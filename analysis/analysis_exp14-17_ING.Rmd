...
tmppath <- paste(path, "/ing_exp14/", sep="")
...
```{r}
exp14 %>%
  group_by(stat, rate, I_e, w_ie, w_ei, trial) %>%
  summarise(M=mean(x), SD=sd(x)) ->
  exp14.m
```

ggplot(data=filter(exp14.m, stat=="isi_e", trial == "19" | trial == "0"), aes(x=I_e, y=M)) + 
  geom_point() + theme_classic() + facet_grid(.~trial)
```
...
tmppath <- paste(path, "/ing_exp15/", sep="")
...
tmppath <- paste(path, "/ing_exp17/", sep="")
...
```{r}
pdf("figures/exp15-17_ing.pdf", width = 8.5, height = 4.5)
laym <- rbind(
  c(1, 2, 3),
  c(4, 5, 6)  
)
