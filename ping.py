...
def model(time, time_stim, w_e, w_i, w_ei, w_ie, seed=42):
    ...
    """ + """
    I_m = w_m * w * (V_K - V) : amp
    dw/dt = (w_backf - w) / tau_w/ms : 1
    w_backf = 1 / (1 + exp(-1 * (V/mV + 35) / 10)) : 1
    ...
    # Syn
    # Ext
    C_back_e = Synapses(P_e_back, P_e, model=syn_e_back, pre='g += w_e', 
                        connect='i == j')
    C_back_i = Synapses(P_i_back, P_i, model=syn_e_back, pre='g += w_i', 
                        connect='i == j')
